...
class pxssh (spawn):
    ...
    def login (self,server,username,password='',terminal_type='ansi',original_prompt=r"[#$]",login_timeout=10,port=None,auto_prompt_reset=True):
        ...
        if i==2: # password or passphrase
            ...
            i = self.expect(["(?i)are you sure you want to continue connecting", original_prompt, "(?i)(?:password)|(?:passphrase for key)", "(?i)permission denied", "(?i)terminal type","Please enter the controller|What Work env, you want to work with", TIMEOUT])
            # To handle cavisson login systems - Ankur Srivastava 
            # Added for handling controller/work selection profile 
