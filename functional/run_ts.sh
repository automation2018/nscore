...
function init() {
    ...
    R_FILE="${R_DIR}/$(basename ${testSuite})_${CYCLENO}.txt"
    XML_FILE="${R_DIR}/$(basename ${testSuite})_${CYCLENO}.xml"
...
function run(){
    ...
    echo "Running TestSuite = '$(basename $testSuite)'"
   
    # running test in backend
    bin/ts_run -n ${testSuite} | tee $TEMP_FILE    
...
function get_test_case_count() {
  ts=$(basename ${testSuite})
  testcases=$(cat testsuites/${ts}.conf |grep -v "#" | cut -d ' ' -f2)
  count=0
  for testcase in ${testcases}
  do
    i=$(grep -ci "^FUNC-" testcases/${testcase}/iteration.spec)
    count=$(($count + $i))
  done
  echo ${count}
}
...
function main() {
    ...
    echo "Total Cases to test in $(basename ${testSuite}) suite = $(get_test_case_count)"
