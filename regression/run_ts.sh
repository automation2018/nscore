...
# Initiates testsuite specific variables values
function init() {
  CUR_DIR=$(pwd)
  ...
  # Making results workbench if not present
...
function run(){
    ...
    echo "Starting NSCore regression automation suite using command bin/ts_run"
    ...
    # Changing back to automation_wdir
    cd - >/dev/null
}
...
function copy_logs(){
    [ ! -d ${CUR_DIR}/logs ] && mkdir -p ${CUR_DIR}/logs
...
# Returns the count of total testcase to run for current testsuite 
