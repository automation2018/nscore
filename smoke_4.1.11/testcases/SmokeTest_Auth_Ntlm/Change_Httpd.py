...
def main ():
    ...
    child.sendline('rm -f /etc/httpd/conf/httpd.conf')
    child.expect(prompt)
    child.sendline('cp /etc/httpd/conf/httpd_%s.conf /etc/httpd/conf/httpd.conf'%(type))
    child.expect(prompt)
    print child.before, child.after
    child.sendline('/etc/init.d/httpd restart')
    child.expect(prompt, timeout=60)
    print child.before, child.after
    time.sleep(10)
    child.sendline('/etc/init.d/httpd  status')
    child.expect(prompt, timeout=60)
    print child.before, child.after
    #time.sleep(60)
    sys.exit(1)
